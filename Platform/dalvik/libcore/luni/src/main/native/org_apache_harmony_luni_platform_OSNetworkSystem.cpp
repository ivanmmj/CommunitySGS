...
//[ TelephonyFeature.CONFIG_MANUAL_ATTACH_FEATURE
extern int checkPdpConnectivity();
//]
...
static int sockConnectWithTimeout(int handle, struct sockaddr_storage addr,
    ...
    if (SOCKET_STEP_START == step) {
        ...
        if (useAdbNetworkingForAddress(&addr)) {
            ...
            #ifdef CONFIG_MANUAL_ATTACH_FEATURE
                checkPdpConnectivity();
            #endif
...
static void osNetworkSystem_setSocketOptionImpl(JNIEnv* env, jclass clazz,
    ...
    switch ((int) anOption & 0xffff) {
        ...
        case JAVASOCKOPT_SO_BINDTODEVICE: {
            //LOGD("Got Socket Set Bind to Device %s", stringVal);

            strcpy(interfaceName, (char*)stringVal);

            lengthInterface = strlen(interfaceName);

            //LOGD("Got Socket Set BINDTODEVICE Interface Name is:%s Interface length is %d ", (char*)interfaceName, lengthInterface);
